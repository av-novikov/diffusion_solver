...
GasOilNITEllipticSolver::GasOilNITEllipticSolver(GasOilNIT_Elliptic* _model) : AbstractSolver<GasOilNIT_Elliptic>(_model)
{
	// Output streams
...
GasOilNITEllipticSolver::~GasOilNITEllipticSolver()
{
	plot_Pdyn.close();
	plot_Sdyn.close();
	plot_qcells.close();
}
void GasOilNITEllipticSolver::writeData()
{
...
void GasOilNITEllipticSolver::control()
{
	writeData();
...
void GasOilNITEllipticSolver::start()
{
	int counter = 0;
	iterations = 8;

	fillIndices();
	solver.Init(2 * (model->cellsNum + model->wellCells.size()), 1e-15, 1e-15);
...
void GasOilNITEllipticSolver::copySolution(const paralution::LocalVector<double>& sol)
{
...
void GasOilNITEllipticSolver::solveStep()
{
	int cellIdx, varIdx;
...
void GasOilNITEllipticSolver::doNextStep()
{
	solveStep();
...
void GasOilNITEllipticSolver::fill()
{
	int counter = 0;
	int i;
...
void GasOilNITEllipticSolver::fillIndices()
{
	int counter = 0;
...
void GasOilNITEllipticSolver::fillq()
{
	int i = 0;
	map<int, double>::iterator it = model->Qcell.begin();
	while (it != model->Qcell.end())
...
void GasOilNITEllipticSolver::fillDq()
{
	for (int i = 0; i < n; i++)
		dq[i] = 0.0;
}
void GasOilNITEllipticSolver::solveDq(double mult)
{
	fillDq();
	filldPdQ(mult);
	solveStep();
...
void GasOilNITEllipticSolver::solveSystem()
{
	double s = 0.0, p1, p2;
	map<int, double>::iterator it;
...
void GasOilNITEllipticSolver::filldPdQ(double mult)
{
	double p1, p2, ratio;

...
AcidRecFracMovSolver::AcidRecFracMovSolver(AcidRecFracMov* _model) : AbstractSolver<AcidRecFracMov>(_model)
{
	...
	pvd_frac.open("snaps/AcidRecFracMov_frac.pvd", std::ofstream::out);
	pvd_poro.open("snaps/AcidRecFracMov_poro.pvd", std::ofstream::out);
...
AcidRecFracMovSolver::~AcidRecFracMovSolver()
{
	delete[] x, y;
...
void AcidRecFracMovSolver::writeData()
{
	...
	pvd_frac << "0\" file=\"AcidRecFracMov_frac_" + std::to_string(step_idx) + ".vtu\"/>\n";
	pvd_poro << "0\" file=\"AcidRecFracMov_poro_" + std::to_string(step_idx) + ".vtu\"/>\n";
...
void AcidRecFracMovSolver::analyzeNewtonConvergence()
{
    DECREASE_STEP = false;
    INCREASE_STEP = true;
...
void AcidRecFracMovSolver::control()
{
	writeData();
    ...
    if (cfl_x + cfl_y + cfl_z > 0.98)
    {
        INCREASE_STEP = false;
        DECREASE_STEP = true;
        MULT_DOWN = (cfl_x + cfl_y + cfl_z) / 0.98;
    }
    else if (INCREASE_STEP && (cfl_x + cfl_y + cfl_z) * MULT_UP > 0.98)
        MULT_UP = 0.98 / (cfl_x + cfl_y + cfl_z);
...
void AcidRecFracMovSolver::start()
{
	step_idx = 0;
	...
	while (cur_t < Tt)
	{
		...
		std::cout << "cfl_x = " << cfl_x << "\tcfl_y = " << cfl_y << "\tcfl_z = " << cfl_z << "\tcfl = " << cfl << std::endl;
...
bool AcidRecFracMovSolver::doNextSmartStep()
{
    init_step_res.clear();
...
void AcidRecFracMovSolver::checkStability()
{
...
void AcidRecFracMovSolver::checkVariables()
{
...
bool AcidRecFracMovSolver::solveSmartStep()
{
	int cellIdx, varIdx;
	err_newton = 1.0;
	...
	while (continueIterations())
	{
        ...
        if (init_step_res.back() >= 2.0 * final_step_res.back())
        {
            ...
            averValue(averVal);
            for (int i = 0; i < AcidRecFracMov::var_size; i++)
...
void AcidRecFracMovSolver::computeJac()
{
...
void AcidRecFracMovSolver::fill()
{

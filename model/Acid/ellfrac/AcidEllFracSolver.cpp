...
AcidEllFracSolver::AcidEllFracSolver(AcidEllFrac* _model) : AbstractSolver<AcidEllFrac>(_model)
{
	...
	pvd_frac.open("snaps/AcidEllFrac_frac.pvd", std::ofstream::out);
	pvd_poro.open("snaps/AcidEllFrac_poro.pvd", std::ofstream::out);
...
AcidEllFracSolver::~AcidEllFracSolver()
{
	delete[] x, y;
...
void AcidEllFracSolver::control()
{
	writeData();
...
void AcidEllFracSolver::doNextStep()
{
	solveStep();
	model->calculateTrans();
}
void AcidEllFracSolver::copySolution(const paralution::LocalVector<double>& sol)
{
...
void AcidEllFracSolver::checkStability()
{
...
void AcidEllFracSolver::solveStep()
{
	int cellIdx, varIdx;
	err_newton = 1.0;
